...
const btUpload = document.getElementById('bt_upload');
...
form.addEventListener('submit',(event) => {
    ...
    const userFile = document.getElementById('file_input').files[0];
    ...
    if(userFile == null){
        window.alert('Selecione um documento!');
    }
    else{
        window.alert('Upload feito!');
    }
...
const gastosDiarios = document.getElementById("gastosDiarios");
document.getElementById("datadash2").addEventListener("change", function() {
    var choosedData = this.value;
    var dateEntered = new Date(choosedData);
    console.log(choosedData); //e.g. 2015-11-13
    console.log(dateEntered); //e.g. Fri Nov 13 2015 00:00:00 GMT+0000 (GMT Standard Time)
    
    const URL = `http://localhost:8080/api/${choosedData}`;
    ...
    gastosDiarios.innerHTML= "R$" +` ${result}`
    })
...
document.getElementById("box2").addEventListener("click", function() {
    ...
    const URL = `http://localhost:8080/api/gastosMes`;
